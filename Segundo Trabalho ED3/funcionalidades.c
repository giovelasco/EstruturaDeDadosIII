...
void Funcionalidade3_6(FILE *dadosBIN, char *nomeCampo){
    registroDados regDados;
...
void Funcionalidade6(char *nomeDadosBIN, char *nomeIndiceBIN, int n){
    ...
    // verifica o status do arquivo de dados e do arquivo de índices
    /*if(VerificaStatusDadosIndice(dadosBIN, indiceBIN) == 0)
        return;*/
    ...
    for(int i = 0; i < n; i++){
        ...
        if(strcmp(nomeCampo, "nomeTecnologiaOrigemDestino") == 0){
            ...
            //printf("%d", BuscaArvoreB(indiceBIN, tecOrigemDestino));
        }
        else{
            Funcionalidade3_6(dadosBIN, nomeCampo);
        }
    }
}
